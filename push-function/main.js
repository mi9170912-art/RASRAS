/*
  ...
  Appwrite، وبتبعت إشعار فوري (Firebase Cloud Messaging) بس
  للأجهزة اللي بتخص صاحب الاعتماد المطلوب (حسب الدور، أو حسب
  الشخص اللي أنشأ الأمر لو الإشعار معلوماتي)، مش لكل الأجهزة.
...
/*
  بترجع أسامي الدخول (usernames) بتاعت أي مستخدم اسمه المعروض
  (name) بيطابق الاسم اللي جاي، عشان نقدر نبعت لصاحب أمر معيّن
  حتى لو معندناش على الأمر نفسه غير اسمه المعروض.
*/
function usernamesForDisplayName(usersConfig, displayName){

  if(!usersConfig || !displayName) return [];

  return Object.keys(usersConfig).filter(
    username => usersConfig[username] && usersConfig[username].name === displayName
  );

}

/*
  بتحدد مين من الأجهزة المسجّلة (tokens) المفروض يوصله الإشعار ده
  بالظبط، حسب الدور (targetRoles) أو صاحب الأمر (targetUser).
  لو الإشعار مش معلّم بأي حاجة (إشعارات قديمة)، بيتبعت للكل زي
  ما كان بيحصل قبل كده.
*/
function tokensForNotification(n, allTokens, usersConfig){

  if(!n.targetRoles && !n.targetUser){

    return allTokens.map(t => t.token).filter(Boolean);

  }

  const matched = new Set();

  if(n.targetRoles && n.targetRoles.length){

    for(const t of allTokens){

      if(t.token && n.targetRoles.includes(t.role)){
  ...
  if(n.targetUser){
    ...
    const usernames = usernamesForDisplayName(usersConfig, n.targetUser);

    for(const t of allTokens){

      if(t.token && usernames.includes(t.username)){
...
module.exports = async ({ req, res, log, error }) => {
  ...
  try{
    ...
    const allTokens = data.fcmTokens || [];

    const usersConfig = data.usersConfig || {};

    log(`لقيت ${notifications.length} إشعار و${allTokens.length} جهاز مسجّل`);

    if(!notifications.length || !allTokens.length){
    ...
    for(const n of newOnes){
      ...
      const targetTokens = tokensForNotification(n, allTokens, usersConfig);

      if(!targetTokens.length){

        log(`الإشعار "${n.title}" مالوش جهاز مستهدف مسجّل - اتخطى`);
      ...
      try{
        ...
        log(`اتبعت إشعار "${n.title}" لـ ${response.successCount} من ${targetTokens.length} جهاز مستهدف`);
